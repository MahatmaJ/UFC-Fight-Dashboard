...
chart3_card = dbc.Card(
    className="ufc-card",
    children=[
        dbc.CardHeader("Control Time vs. Win Method & Win Rate"),
...
def update_control_vs_method(year_range, weight_class):
    dff = filter_df(df, year_range, weight_class)
    ctrl_df = dff[dff["has_winner"] & dff["winner_ctrl_sec"].notna()]
    ctrl_df = ctrl_df[ctrl_df["method_group"] != "Overturned/No Winner"]
    present_order = [m for m in METHOD_ORDER if m in ctrl_df["method_group"].unique()]
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("Winner's Control Time by Method", "Win Rate by Own Control Time"),
        column_widths=[0.55, 0.45],
    )

    for method in present_order:
        sub = ctrl_df[ctrl_df["method_group"] == method]
        fig.add_trace(
            go.Box(y=sub["winner_ctrl_sec"], name=method, marker_color=METHOD_COLORS[method],
                   showlegend=False),
            row=1, col=1,
        )
    ...
    fig.add_trace(
        go.Bar(
            x=bucket_summary["ctrl_bucket"], y=bucket_summary["win_rate"],
            marker_color=UFC_RED_BRIGHT, showlegend=False,
            text=[f"{v:.0f}%" if not pd.isna(v) else "" for v in bucket_summary["win_rate"]],
            textposition="outside",
        ),
        row=1, col=2,
    )
    fig.add_hline(y=50, line_dash="dash", line_color=UFC_TEXT_MUTED, row=1, col=2)

    fig.update_yaxes(title_text="Seconds", row=1, col=1)
    fig.update_yaxes(title_text="Win Rate (%)", range=[0, 100], row=1, col=2)
    fig.update_xaxes(title_text="Own Control Time", row=1, col=2)
    fig.update_layout(**PLOTLY_DARK_LAYOUT)
    fig.update_annotations(font_color=UFC_TEXT_MUTED, font_size=12)
    return fig
...
def update_stance_winrate(year_range, weight_class, view_mode):
    lo, hi = year_range
    dff = long_df_valid[(long_df_valid["year"] >= lo) & (long_df_valid["year"] <= hi)]
    ...
    if view_mode == "overall":
        summary = dff.groupby("stance").agg(win_rate=("won", "mean"), n=("won", "size")).reset_index()
    ...
    # By weight class -> heatmap
    order = [w for w in WEIGHT_CLASSES if w in dff["weight_class"].unique()]
    pivot_rate = dff.pivot_table(index="weight_class", columns="stance", values="won",
                                  aggfunc="mean", observed=True).reindex(order)[MAIN_STANCES] * 100
    pivot_n = dff.pivot_table(index="weight_class", columns="stance", values="won",
                               aggfunc="size", observed=True).reindex(order)[MAIN_STANCES]

    text = pivot_rate.copy().astype(object)
    for r in pivot_rate.index:
        for c in pivot_rate.columns:
            rate, n = pivot_rate.loc[r, c], pivot_n.loc[r, c]
            text.loc[r, c] = f"{rate:.0f}%<br>n={int(n)}" if pd.notna(rate) else ""

    fig = go.Figure(
        data=go.Heatmap(
            z=pivot_rate.values, x=MAIN_STANCES, y=order,
            text=text.values, texttemplate="%{text}",
            colorscale=[[0, "#3a1010"], [0.5, UFC_RED], [1, UFC_GOLD]],
            zmin=30, zmax=70,
            colorbar=dict(title="Win %", tickfont=dict(color=UFC_TEXT)),
        )
    )
    layout = dict(PLOTLY_DARK_LAYOUT)
    layout.update(xaxis_title="Stance", yaxis_title="")
    fig.update_layout(**layout)
    return fig
